...
// Questions fréquentes suggérées avec catégories
const SUGGESTED_QUESTIONS = [
  { text: "Quels sont les emplois du temps disponibles ?", category: "Emploi du temps" },
  { text: "Comment consulter mes résultats ?", category: "Résultats" },
  { text: "Quels documents administratifs sont disponibles ?", category: "Documents" },
  { text: "Quand commence la période de stage ?", category: "Stage" },
  { text: "Où trouver les supports de cours ?", category: "Cours" },
  { text: "Quelles sont les dates des EFM régionaux ?", category: "Examens" },
  { text: "Comment contacter l'établissement ?", category: "Contact" }
]
...
export default function Chat() {
  ...
  return (
    <div className="chat-root">
      {/* SmartPresence Navbar */}
      <nav className="sp-navbar">
        <div className="sp-navbar-container">
          {/* Logo */}
          <div className="sp-navbar-brand">
            <div className="sp-logo">SP</div>
            <span className="sp-brand-name">Smart Presence AI</span>
          </div>
          
          {/* Navigation Tabs */}
          <div className="sp-navbar-tabs">
            <button className="sp-tab">🏠 Accueil</button>
            <button className="sp-tab sp-tab-active">🤖 Assistant NTIC2</button>
            <button className="sp-tab">👤 Admin</button>
          </div>
          
          {/* User Section */}
          <div className="sp-navbar-user">
            <span className="sp-status-badge">
              <span className={`sp-status-dot ${status.connected ? 'connected' : ''}`}></span>
              {Number(status.chunks) || 0} chunks
            </span>
            <button className="sp-icon-btn" onClick={clearConversation} title="Effacer">
              🗑️
            </button>
            <div className="sp-user-avatar">B</div>
          </div>
        </div>
      </nav>
      ...
      <div className="chat-window">
        {/* Afficher les suggestions en mode chips horizontaux */}
        {messages.length === 1 && (
          <div className="suggestions-container-initial">
            <div className="suggestions-grid-horizontal">
              {SUGGESTED_QUESTIONS.map((item, idx) => (
                <button
                  key={idx}
                  className="suggestion-chip-horizontal"
                  onClick={() => {
                    setInput(item.text)
                    setTimeout(() => send(item.text), 100)
                  }}
                  disabled={loading}
                >
                  {item.text}
